...
sed -i "s/address_data:/address_data: $PDOT_ADDRESS/" pdot-latency-cold-stubby.yml
sed -i "s/address_data:/address_data: $PDOT_ADDRESS/" pdot-latency-warm-stubby.yml
...
cp ../cloudflare-stubby-template.yml cloudflare-latency-cold-stubby.yml
cp ../cloudflare-stubby-template.yml cloudflare-latency-warm-stubby.yml
sed -i "s/idle_timeout:/idle_timeout: 0/" cloudflare-latency-cold-stubby.yml
sed -i "s/idle_timeout:/idle_timeout: 9000/" cloudflare-latency-warm-stubby.yml
...
do
  cd $BIN_DIR
  sudo -b ./cloudflare-stubby -C cloudflare-latency-cold-stubby.yml -g > /dev/null 2>&1 &
  sleep 2
  cd $EVAL_DIR
...
do
  cd $BIN_DIR
  sudo -b ./cloudflare-stubby -C cloudflare-latency-warm-stubby.yml -g > /dev/null 2>&1 &
  sleep 2
  cd $EVAL_DIR
